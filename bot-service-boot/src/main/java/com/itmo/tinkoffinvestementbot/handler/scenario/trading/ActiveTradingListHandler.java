...
public class ActiveTradingListHandler implements CallbackUpdate, TextUpdate, BotStateTypeHandler {
    ...
    public void handleText(User user, Message message) {
        ...
        switch (user.getBotState()) {
            case TRADINGS_LIST -> {
//                List<ResponseActiveStrategyDto> allWiki = strategyService.getActiveStrategies();
                sendMessage.setText("На этой фиче разработчики заснули");
...
//                );
    ...
    public List<BotState> getSupportedBotStates() {
        return List.of(
                BotState.TRADINGS_LIST, BotState.STRATEGY_WIKI
        );
    }
}
